...
| File | What it is |
|---|---|
| [`coroutine-parallelism-architecture.md`](coroutine-parallelism-architecture.md) | **The migration design** — replacing OpenMP + the Python driver with a Boost.Cobalt coroutine spine over {thread-pool, NEON, fused-MLX}. Code-level inventory of all 64 parallel regions; **root-causes the CX1 arm64 `-t>1` bug** + a fix-by-construction; per-region substrate/overlap table; ranked wins (restore multicore ≫ kill fork/exec ≫ overlap); phased plan with falsifiable hypotheses + kill criteria. |
