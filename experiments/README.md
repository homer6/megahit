...
| **H15** | Foundation batch primitives for the rest of `assemble`: `OutgoingEdgesBatch`/`IncomingEdgesBatch` (`kFlagWriteOut`) + `EdgeOutdegreeZeroBatch`/`EdgeIndegreeZeroBatch` (`kFlagMustEq0`). | integration · [#4] | **2.05× / 1.89× in isolation**, all `== scalar` (PASS) — but **REVERTED** after H16 showed the integration regresses end-to-end. Construction is sound; the lever just doesn't pay on the small/repeated sweeps it would feed. See [`h15-outgoing-edges-batch/`](h15-outgoing-edges-batch/README.md). |
| **H16** | Wire H15 into the rest of `assemble` (tip/low-depth/weak-link/bubble + `sdbg_pruning`). | integration · [#4] | **DISPROVEN — 1.41× SLOWER**, reverted. Bit-identical (correctness fine), but `sdbg_pruning::Trim` radix-sorts all 30.7 M edges ~6× for a cheap early-exiting filter → **+16 s for zero gain**; collapsed-graph simplifiers neutral. **The first sweep (single large scatter) was the only real win.** See [`h16-simplifier-batch-integration/`](h16-simplifier-batch-integration/README.md). |
